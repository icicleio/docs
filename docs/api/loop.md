...
- [Loop Functions](#loop-functions) - Functions for accessing the actual event loop instance.
    - [loop()](#loop) - Returns the loop instance or initialize with an instance of `Icicle\Loop\LoopInterface`.
    ...
    - [signal()](#signal) - Creates a process signal listener, returning an `Icicle\Loop\Events\SignalInterface` object.
    - [isEmpty()](#isempty) - Determines if there are any pending events in the loop.
    - [reInit()](#reinit) - Re-initializes the loop after a process is forked.
    - [clear()](#clear) - Removes all pending events from the loop.
- [Loop Implementations](#loop-implementations) - Available loop implementations.
- [Throwing Exceptions](#throwing-exceptions) - Throwing exceptions from callbacks.
- [Events](#events) - An event object is created when an event is scheduled in the loop.
    ...
    - [TimerInterface](#timerinterface) - Executes a callback after a interval of time has elapsed.
        - [start()](#start) - Starts the timer if it was stopped.
    ...
    - [ImmediateInterface](#immediateinterface) - Executes a callback once no other events are active in the loop.
        - [execute()](#execute) - Execute the immediate again if it has already been executed.
        - [isPending()](#ispending-2) - Determines if the immediate is pending.
    - [SignalInterface](#signalinterface) - Listens for process signals.
        - [enable()](#enable) - Enables the signal listener if previously disabled.
        - [disable()](#disable) - Disables the signal listener.
        - [isEnabled()](#isenabled) - Determines if listener is enabled.
        - [getSignal()](#getsignal) - Returns the signal number that will trigger the callback when received.
